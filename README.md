...
Google's AI ecosystem moves faster than its own documentation. Names change,
SDKs get replaced, pages move hosts — and the search results keep pointing at
all the old versions.

This repository is a practical guide for developers who want to build with the
Google GenAI ecosystem without spending most of their time figuring out where
to start. It organizes the documentation that matters, untangles the naming,
and lays out implementation paths by use case.
...
|---|---|---|
| 00 | [orientation](00-orientation/) | The ecosystem map, the nomenclature decoder, and what you need before starting — per path |
...
| 04 | [development-with-models](04-development-with-models/) | Building against the SDK: which call surface to write, calling without the SDK, reliable design, configuration, errors, retries, timeouts, testing. Python and JavaScript examples will live here |
...
The example directories are still empty — prose first, code as each module
lands. The environment is already real, though: Python 3.12+ managed with
[uv](https://docs.astral.sh/uv/), and the `google-genai` SDK declared as a
dependency.
...
When examples land, every one will run as-is with nothing but an API key in
your environment. If one doesn't, that's a bug — please open an issue.
...
nomenclature decoder, and prerequisites; the [glossary](12-glossary/); the
[first learning path](10-learning-paths/00-first-steps-with-gemini.md); and the
[resources](resources/) module — the source catalog, the reference
repositories, and the ecosystem changelog.

Current focus is [foundations](01-foundations/).
